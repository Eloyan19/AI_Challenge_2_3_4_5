...
**Главный принцип:** не управляй поведением ВНУТРИ Plan Mode из CLAUDE.md.
Plan Mode впрыскивает свой шаблон («Phase 1 = Explore») свежей императивной инструкцией — она всегда перебьёт фоновое правило CLAUDE.md. Поэтому единственная рабочая точка контроля — решение вызывать `EnterPlanMode` или нет. В этот момент шаблона Plan Mode ещё нет в контексте, и CLAUDE.md — единственный авторитет.

**ГЕЙТ — проверяй ПЕРЕД каждым вызовом `EnterPlanMode`:**

Нужен ли задаче доменный эксперт? (по таблице маршрутизации первыми идут ARCHITECT / LLM ENGINEER / SECURITY AUDITOR / PERFORMANCE ENGINEER / DEBUG SPECIALIST)

- **ДА → `EnterPlanMode` сейчас ЗАПРЕЩЁН.**
  1. Сначала запусти доменного эксперта в обычном режиме. Промпт ограничь: *"до 200 слов, только ключевые архитектурные решения и риски"*.
  2. Дождись его вывода — он должен оказаться в контексте.
  3. Только ПОСЛЕ этого вызывай `EnterPlanMode`.
  4. Внутри Plan Mode — следуй его шаблону как есть. Explore-first теперь безвреден: инвариант «эксперт раньше всего» уже выполнен. НЕ пытайся переупорядочивать фазы Plan Mode изнутри.

- **НЕТ (чистый DEVELOPER, архитектура не меняется) →** можно сразу `EnterPlanMode`.

**Почему так, а не «приоритет маршрутизации»:** прежнее правило требовало запускать эксперта «до Explore» уже находясь внутри Plan Mode — то есть нарушать его активный шаблон. Это структурно невыдерживаемо: inline-шаблон Plan Mode всегда свежее и конкретнее фонового CLAUDE.md. Новый порядок убирает сам конфликт — эксперт это предусловие входа, а не действие внутри.

**Когда вообще нужен Plan Mode** (если гейт пройден) — хотя бы одно условие:
...
В Plan Mode: читай код, задавай уточняющие вопросы, опиши план (какие файлы, что меняется, в каком порядке). Выходи только после явного одобрения пользователя.
...
**Ловушка рационализации:** обсуждение задачи в чате — не замена прохождению гейта. Сложность определяется по коду и архитектуре, а не по длине разговора. Фраза «давай реализуем» / «да, делаем» — триггер запустить ГЕЙТ (эксперт → потом Plan Mode), а не код напрямую и не Plan Mode напрямую.

Plan-агент — общий архитектор кода, он не знает специфики протоколов, LLM-паттернов, угроз безопасности. Доменный эксперт даёт ему эти знания в сжатом виде — без него Plan-агент спроектирует технически правильное, но архитектурно неверное решение.
...
### Два уровня агентов — важно не путать

**Уровень 1 — Persona-агенты** (из routing table ниже):
Эксперты с характером, знаниями и системным промптом. Вызываются явно через `Agent(model: "...", prompt: "Ты ARCHITECT/DEVELOPER/...")`. Один агент смотрит на задачу **целиком** как эксперт — думает широко, видит архитектурный контекст, выдаёт связный анализ.
Примеры: ARCHITECT, ANDROID DEVELOPER, CODE REVIEWER, BACKEND DEVELOPER.

**Уровень 2 — Skill-агенты** (внутренние, запускаются скиллами):
Технические сканеры — каждый проверяет **один узкий угол** (null-deref, race conditions, эффективность). Нет personas, нет широкого контекста. Запускаются **автоматически внутри скилла** — оркестратор их не вызывает напрямую.
Примеры: finder-correctness, finder-robustness — внутри `/code-review`.

**Когда что использовать:**
```
Быстрое ревью в цепочке (DEVELOPER → REVIEWER)?
  └─► Agent(CODE REVIEWER)  — один агент, целостный взгляд, ~2 мин

Тщательное ревью перед мержем / важная фича?
  └─► Skill("code-review", "medium")  — скилл сам запускает 8+ angle-агентов + верификаторов
```
...
| `sonnet` | ANDROID DEVELOPER, CODE REVIEWER, UI/UX SPECIALIST, BACKEND DEVELOPER | Основная разработка и ревью |
...
**Когда вызывать:** написание MCP-серверов, REST/HTTP API, деплой на VPS, интеграции с внешними сервисами (почта, календарь, мессенджеры и т.д.)  
**Фокус:**
- **MCP-серверы** — FastMCP, транспорты (stdio, streamable-http, SSE), структура tools/resources/prompts
- **Python** — async/await, aiohttp, httpx, pydantic, type hints
- **VPS-деплой** — systemd сервисы, nginx reverse proxy, SSL (certbot), переменные окружения, .env файлы
- **Внешние API** — OAuth2 (Google, Microsoft), REST, rate limits, retry логика, обработка ошибок
- **Безопасность** — не хранить секреты в коде, HTTPS only, валидация входных данных

**Prompt-ядро:**
> Ты Senior Python Backend Developer, специализируешься на MCP-серверах и интеграциях.
> Пиши чистый, минималистичный Python. Используй FastMCP если нет причин писать на голом SDK.
> Думай о деплое с первого дня: конфиги через env vars, логи в stdout, graceful shutdown.
...
Задача получена
     ...
     │    └─► LLM ENGINEER
     │
     └─ "mcp сервер" / "задеплоить" / "интеграция с почтой/календарём/сервисом"
          └─► LLM ENGINEER (дизайн инструментов) → BACKEND DEVELOPER (реализация сервера) → ANDROID DEVELOPER (клиент в приложении)
```
...
7. **`/code-review` — запускай автоматически** после завершения любого блока изменений который затрагивает 2+ файла с логикой. Используй `Skill("code-review")` с уровнем `medium`. Не жди явной просьбы пользователя — это часть стандартного workflow. Исключения: правки только в CLAUDE.md / документации, чисто механический rename.
8. **Explore — запускай автоматически** в следующих ситуациях (до начала работы, параллельно с другими агентами если можно):

   | Ситуация | Что искать | Пример запроса к Explore |
   |---|---|---|
   | Фиксишь паттерн-баг (else, null-check, reset) | Все другие места с тем же паттерном | `grep "else -> Unit" в MainViewModel.kt` |
   | Меняешь сигнатуру функции / интерфейса | Все call sites | `grep "имяФункции("` по проекту |
   | Добавляешь реализацию интерфейса / стратегии | Все существующие реализации для понимания конвенции | `find файлы с : ИмяИнтерфейса` |
   | Изменяешь DI-модуль / @Provides | Все места где inject-ится этот тип | `grep "@Inject" + имя типа` |
   | Рефакторинг shared utility (extension, sealed class) | Все импорты и usages | `grep "import ... ИмяКласса"` |

   **Не нужен Explore:** если задача — 1 файл с очевидным изменением, или ты уже читал нужные файлы в этом сеансе.
