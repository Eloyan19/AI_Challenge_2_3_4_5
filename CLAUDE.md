...
**Ловушка рационализации:** обсуждение задачи в чате — не замена Plan Mode. Сложность задачи определяется по коду и архитектурным решениям, а не по длине разговора. Фраза пользователя «давай реализуем» / «да, делаем» — триггер запустить маршрутизацию, а не код напрямую.

**В Plan Mode — применяй таблицу маршрутизации:**

Посмотри на таблицу маршрутизации ниже и определи кто идёт первым в цепочке для этой задачи:

- **Первый агент — доменный эксперт** (ARCHITECT, LLM ENGINEER, SECURITY AUDITOR, PERFORMANCE ENGINEER, DEBUG SPECIALIST):
  → запусти его **до** Explore/Plan с одним конкретным вопросом об архитектурных ограничениях.
  → в промпте явно ограничь ответ: *"до 200 слов, только ключевые архитектурные решения и риски"*
  → передай этот краткий вывод в контекст Plan-агента

- **Первый агент — DEVELOPER** (задача с ясной реализацией, архитектура не меняется):
  → Explore + Plan достаточно, эксперт не нужен

**Ловушка Plan Mode + шаблон:** Plan Mode имеет встроенный шаблон (Phase 1 = Explore). Этот шаблон НЕ отменяет маршрутизацию из CLAUDE.md. Маршрутизация имеет приоритет:
- Если задача требует доменного эксперта — он запускается **первым**, в том числе до Explore-агентов
- Параллельные агенты (эксперт + Explore) запускаются в **одном сообщении** — не в двух отдельных
- Типичная ошибка: запустить Explore первым «потому что так написано в Plan Mode», а потом доменного эксперта — это нарушение порядка

Plan-агент — общий архитектор кода, он не знает специфики протоколов, LLM-паттернов, угроз безопасности. Доменный эксперт даёт ему эти знания в сжатом виде — без него Plan-агент может спроектировать технически правильное, но архитектурно неверное решение.
