...
Пользователь отправляет сообщение
          ↓
  DetectComplexityUseCase (flash, maxTokens=10)
    → "SIMPLE" → обычный agent.run()
    → "COMPLEX" ↓
          ↓
  RunAgentSwarmUseCase (parallel):
    ...
    CRITIC   — оценивает план, ищет риски
          ↓
  UI: TaskStateCard показывает план + оценку Critic
  Состояние: AWAITING_INPUT — ждём подтверждения
          ↓
  ...
  «Одобрить»: EXECUTION
    RunAgentSwarmUseCase (parallel):
      ...
      VALIDATOR — проверяет, что результат соответствует задаче
            ↓
    JUDGE — финальный выбор: PASS / FAIL
            ↓
    PASS → DONE → ответ добавляется в чат
    FAIL → REPLANNING → новый план → вернуться к AWAITING_INPUT

  «Отклонить»: REPLANNING
    PLANNER + CRITIC снова, с учётом причины отклонения
          ↓
    Новый план → вернуться к AWAITING_INPUT
```
...
| `Execution` | Parallel-запросы к EXECUTOR и VALIDATOR | Спиннер «Выполняю план...» |
| `Validation` | JUDGE анализирует результаты | Спиннер «Валидирую результат...» |
| `Replanning` | Причина отклонения → новый план | Спиннер «Перепланирую...» |
| `Done` | Финальный ответ записан в чат | Зелёная галочка + первая строка ответа |
| `Error` | Что-то пошло не так | Красный текст ошибки + «Закрыть» |
...
Пока задача в оркестраторе (не `Idle`), поле ввода заблокировано — нельзя отправить новое сообщение пока не завершится или не отменится текущая задача.
...
| Роль | Задача | Модель |
|---|---|---|
| `PLANNER` | Разбить задачу на чёткие шаги | `_selectedModel` пользователя |
| `CRITIC` | Найти риски и слабые места в плане | `_selectedModel` пользователя |
| `EXECUTOR` | Выполнить план и сформулировать ответ | `_selectedModel` пользователя |
| `VALIDATOR` | Проверить, что ответ решает задачу | `backgroundModel` (flash) |
| `JUDGE` | Финальное решение PASS/FAIL на основе Executor+Validator | `backgroundModel` (flash) |
...
MainViewModel
├── sendMessage()          — детектирует сложность, ветвит на simple/orchestrator
├── confirmPlan()          — EXECUTION → persistOrchestratorResult
├── rejectPlan(reason)     — REPLANNING → новый план
├── dismissTaskState()     — сброс в Idle + clearTaskPlan
...
domain/                              ← ничего из data не импортирует
  model/
    ...
    TaskState.kt        ← NEW: sealed class машины состояний оркестратора
...
- Room v6 (chat_messages, summary, sticky_facts, branches, working_memory, long_term_memory, user_profiles, task_plan)
