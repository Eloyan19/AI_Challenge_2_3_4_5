...
Приложение поддерживает подключение внешних инструментов через [MCP](https://modelcontextprotocol.io/) — стандартный протокол, позволяющий LLM обращаться к инструментам на удалённом сервере. На VPS запущены **два независимых MCP-сервера** с разными задачами.
...
### Два MCP-сервера на jorchik.com

| Сервер | URL | Порт | Задача |
|---|---|---|---|
| **market** | `jorchik.com/mcp/market` | 8001 | Сбор и анализ рыночных данных |
| **notify** | `jorchik.com/mcp/notify` | 8002 | Уведомления и конвертация валют |

Оба сервера написаны на Python + FastMCP, задеплоены как systemd-сервисы, доступны по HTTPS через nginx с Bearer-токен аутентификацией.

Инфраструктура: [github.com/Eloyan19/mcp-server](https://github.com/Eloyan19/mcp-server)
...
|---|---|
| `start_monitoring(interval)` | Запустить периодический сбор MOEX IMOEX + BTC/USD. Поддерживает «каждые 30 минут», «every hour» и т.д. Первый тик — немедленно |
| `stop_monitoring()` | Остановить сбор (данные в БД сохраняются) |
| `get_monitoring_status()` | Активен ли мониторинг, интервал, кол-во снапшотов |
| `get_current_prices()` | Живая цена прямо сейчас (не пишет в БД) |
| `get_market_summary(hours)` | Сырые снапшоты за последние N часов (список точек) |
| `get_market_stats(hours)` | Агрегированная статистика: min/max/avg/change/change_pct |

**notify-сервер:**

| Инструмент | Что делает |
|---|---|
| `send_telegram_message(text)` | Отправить текст в Telegram владельца бота |
| `get_usd_rub_rate()` | Курс USD/RUB и EUR/RUB от ЦБ РФ (бесплатно, без ключей) |

### Демонстрационный пайплайн (multi-server)

Ключевой сценарий — LLM самостоятельно выстраивает цепочку из инструментов разных серверов:

```
Пользователь: "Проверь рынок за сутки и если BTC упал — 
               переведи цену в рубли и пришли алерт в Telegram"

LLM вызывает:
  1. get_market_stats(hours=24)   [market] → change_pct = -1.8% → триггер
  2. get_usd_rub_rate()           [notify] → 1 USD = 88.5 RUB
  3. send_telegram_message(...)   [notify] → "⚠️ BTC упал на 1.8%\nЦена: $60 211 = 5 328 673 ₽"
```

Три шага, два сервера — LLM сам решает порядок на основе docstrings инструментов. Никакой явной конфигурации пайплайна нет.

**Другие примеры запросов:**
- `"Начни мониторинг рынка каждые 30 минут"` → `start_monitoring("каждые 30 минут")` [market]
- `"Какая сейчас цена биткоина в рублях?"` → `get_current_prices()` [market] → `get_usd_rub_rate()` [notify]
- `"Пришли сводку рынка за неделю в Telegram"` → `get_market_stats(hours=168)` [market] → `send_telegram_message(...)` [notify]

### Архитектура Android-клиента

```
McpServerManager (@Singleton)
  ├── clients["market"] = McpClient("jorchik.com/mcp/market")
  │     ├── Lazy-инициализация сессии (mcp-session-id)
  │     └── Кеш схем 5 минут
  ├── clients["notify"] = McpClient("jorchik.com/mcp/notify")
  │     ├── Lazy-инициализация сессии
  │     └── Кеш схем 5 минут
  ├── allTools() — опрашивает оба сервера, строит toolName → serverId map
  └── callTool(name, args) — роутит к нужному клиенту по имени инструмента
       │
       ▼
  ToolRegistry (@Singleton)         ← нейтральный агрегатор
  ├── allTools() = localTools + mcpServerManager.allTools()
  └── execute(toolCall):
        MCP-инструмент → McpServerManager.callTool()  ← роутит к market или notify
        Локальный       → ToolExecutor.execute()
       │
       ▼
  SimpleAgent.buildLlmRequest()
  └── tools = toolRegistry.allTools()  ← 3 локальных + 8 market + 2 notify
```

**Ключевые свойства McpServerManager:**
- `toolToServer` — `@Volatile` immutable Map, атомарная замена при обновлении (thread-safe)
- При вызове `callTool()` до `allTools()` — автоматический refresh (защита от race condition)
- Логирование роутинга: `D/McpServerManager: callTool → routing 'send_telegram_message' to server 'notify'`

### Отладка MCP в Logcat

| Тег | Что показывает |
|---|---|
| `McpServerManager` | `allTools()` — список всех инструментов с серверов; `callTool` — на какой сервер направлен вызов |
| `McpClient` | `initialize` — открытие сессии; `tools/call →` запрос; `← tools/call` — ответ с временем |

### Добавление нового MCP-инструмента

Приложение обновлять не нужно — достаточно добавить функцию на нужном сервере:

```python
# На market-сервере (/opt/mcp/market/server.py):
@mcp.tool()
async def get_stock_price(ticker: str) -> str:
    """Get current price for a MOEX stock ticker (e.g. SBER, GAZP, YNDX)"""
    ...
```

После `systemctl restart mcp-market` при следующем `allTools()` приложение автоматически увидит новый инструмент.
...
└── SimpleAgent
        ...
        └── ToolRegistry                 ← агрегатор: localTools + MCP tools
                ...
                └── McpServerManager     ← роутер: toolName → serverId → McpClient
                      ├── McpClient("jorchik.com/mcp/market")  ← market tools
                      └── McpClient("jorchik.com/mcp/notify")  ← notify tools
...
# MCP-сервера на jorchik.com (Bearer-токен для авторизации)
MCP_API_KEY=ваш_токен
