...
|---|---|---|---|
| `get_weather` | Текущая погода для любого города | [Yandex Weather](https://developer.tech.yandex.ru/services/38) + [Nominatim](https://nominatim.org/) | нужен для Яндекс.Погоды (бесплатный, 50 запросов/день) |
    ...
    └── MainViewModel            — держит агента, управляет состоянием
            └── SimpleAgent      — ядро: память + LLM-логика + цикл инструментов
                    ...
                    └── ToolExecutor                  — выполнение инструментов
                            ├── Nominatim (OSM)       — геокодинг
                            ├── Yandex Weather API    — погода
...
# Опционально — для инструмента get_weather
# Ключ получить на https://developer.tech.yandex.ru/services/38 (бесплатно, 50 запросов/день)
YANDEX_WEATHER_KEY=...
...
- DeepSeek API, Nominatim, Yandex Weather, Frankfurter, Yandex XML Search
